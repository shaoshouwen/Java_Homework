...
select f_name as 水果 from fruits;
-- 从fruits表中获取f_name和f_price两列
-- 该语句使用SELECT声明从fruits表中获取名称为f_name和f_price两个字段下的所有水果名称和价格
select f_name as 水果名称 ,f_price 单价 from fruits;
-- 查询价格为10.2元的水果的名称
select f_name as 水果名称 from fruits where f_price =10.2;
-- 查找名称为“apple”的水果的价格
select f_price as 价格 from fruits where f_name = 'apple';
-- 查询价格小于10的水果的名称
select f_name as 水果名称 from fruits where f_price <10;
-- 查询s_id为101和102的记录
...
-- 根据s_id对fruits表中的数据进行分组，将每个供应商的水果名称显示出来
 select s_id ,f_name  from fruits  where s_id  in (select  s_id from fruits group by s_id) order by s_id ;
